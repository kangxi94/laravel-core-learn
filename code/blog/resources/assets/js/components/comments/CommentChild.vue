<template>
    <div class="col-md-12" :class="{ padding : show_level }">
        <h5>
          ...
          <span style="color:#37f">{{comment['owner'].name}}</span>:</h5>
        <h5>{{comment.body}}</h5>
    ...
    export default{
        ...
        methods:{
            ...
            is_follow(){
             ...
             post_comment(){
                axios.post('/api/post/'+this.post_id+'/comment',{'parent_id':this.comment.id,'body':this.comment_content}).then((response)=>{
